...
class DockedWindows(Frame):
	def __init__(self, parent, G):
		...
		self.script_dir = os.path.dirname(os.path.abspath('DockedWindows.py'))
	...
	"""--------------------------------------------------ANALYSIS WINDOW FRAMEWORK----------------------------------------------------"""
	
	#Attempt at making universal docked window frame to display analysis results
	
	def AnalysisWindow(self,AnalysisName,photoName):
		if \
		  (not hasattr(self, 'AnalysisFrame') or self.AnalysisFrame.winfo_exists() == 0) and \
		  (not hasattr(self, 'AnalysisPopup') or self.AnalysisPopup.winfo_exists() == 0):
			# mini frame to display node degree analysis graph:
			self.frameOrWindow = 0 # 0 - frame, 1 - popup window
			self.AnalysisFrame = Frame(self.parent, height=200, width=200, bg='white', borderwidth=3, relief='raised')
			self.AnalysisFrame.pack_propagate(0)
			self.AnalysisFrame.pack(side='left', anchor='nw', fill='y')

			# toolbar to store max, min, exit buttons
			self.toolbarFrame = Frame(self.AnalysisFrame, bg='light gray')
			self.toolbarFrame.pack(side='top', fill='x')

			# toolbar label
			analysisToolbarLabel = Label(self.toolbarFrame, text=AnalysisName, bg='light gray')
			analysisToolbarLabel.pack(side='left')

			# creates images for the buttons on the toolbar
			image = Image.open(os.path.join(self.script_dir,"exit.png")) 
			self.exitImage = ImageTk.PhotoImage(image, master=self.toolbarFrame)
			exitButton = Button(self.toolbarFrame, image=self.exitImage, highlightbackground='light gray', command=self.analysisExitU)
			image = Image.open(os.path.join(self.script_dir,"minimize.png"))
			self.minImage = ImageTk.PhotoImage(image, master=self.toolbarFrame)
			minButton = Button(self.toolbarFrame, image=self.minImage, highlightbackground='light gray', \
			command= lambda: self.analysisMinU(AnalysisName,photoName))
			image = Image.open(os.path.join(self.script_dir,"maximize.png"))
			self.maxImage = ImageTk.PhotoImage(image, master=self.toolbarFrame)
			maxButton = Button(self.toolbarFrame, image=self.maxImage, highlightbackground='light gray', \
			command= lambda: self.analysisMaxU(AnalysisName,photoName))

			exitButton.pack(side='right')
			maxButton.pack(side='right')
			minButton.pack(side='right')

			image = Image.open(os.path.join(self.script_dir, photoName))
			photo = ImageTk.PhotoImage(image, master=self.AnalysisFrame)

			# displays figure on a label in the frame
			label = Label(self.AnalysisFrame, image=photo, bg="white")
			label.image = photo
		...
		# updates node degree graph if tab is pressed again
			...
			self.AnalysisWindow(AnalysisName,photoName)
		
		elif hasattr(self, 'frameOrWindow') and self.frameOrWindow == 1:
			self.AnalysisPopup.destroy()
			self.AnalysisWindow(AnalysisName,photoName)

	# destroys either the node frame or popup when exit button is pressed
	def analysisExitU(self):
		if self.frameOrWindow == 0:
			self.AnalysisFrame.destroy()
		else:
			self.AnalysisPopup.destroy()
		self.appendLog("Analysis Exited")
	
	# docks node popup window into a frame or minimizes frame into just the toolbar when minimize button is pressed
	def analysisMinU(self,AnalysisName,photo):
		if self.frameOrWindow == 1:
			self.AnalysisPopup.destroy()
			self.AnalysisWindow(AnalysisName,photo)
		else:
			self.AnalysisFrame.config(height='30')
			self.AnalysisFrame.pack_configure(anchor='sw', fill='none')
		self.appendLog("Analysis Minimized")
	
	# maximizes toolbar into a docked frame or maximizes frame into a popup window when maximize button is pressed
	def analysisMaxU(self,AnalysisName,photoName):
		if self.frameOrWindow == 0 and self.AnalysisFrame.winfo_height() > 30:
			self.AnalysisFrame.destroy()		

			# popout menu to display node degree analysis graph:
			self.frameOrWindow = 1
			self.AnalysisPopup = Toplevel(self.parent, bg='white')
			self.AnalysisPopup.title(AnalysisName)
			self.AnalysisPopup.overrideredirect(1)
			self.AnalysisPopup.geometry(("%dx%d%+d%+d" % (600, 550, 200, 100)))

			# create a frame that makes the toolbar for min, max, and exit buttons
			analysisToolbar = Frame(self.AnalysisPopup, bg='light gray')
			analysisToolbar.pack(side='top', fill='x')
			analysisToolbar.bind('<ButtonPress-1>', self.dragWindowStart)
			analysisToolbar.bind('<ButtonRelease-1>', lambda event: self.dragWindowEnd(event, self.AnalysisPopup))

			# toolbar label
			analysisToolbarLabel = Label(analysisToolbar, text=AnalysisName, bg='light gray')
			analysisToolbarLabel.pack(side='left')

			# creates images for toolbar buttons
			image = Image.open(os.path.join(self.script_dir,"exit.png"))
			self.exitImage2 = ImageTk.PhotoImage(image, master=analysisToolbar)
			exitButton = Button(analysisToolbar, image=self.exitImage2, highlightbackground='light gray', command=self.analysisExitU)
			image = Image.open(os.path.join(self.script_dir,"minimize.png"))
			self.minImage2 = ImageTk.PhotoImage(image, master=analysisToolbar)
			minButton = Button(analysisToolbar, image=self.minImage2, highlightbackground='light gray', command= \
			lambda: self.analysisMinU(AnalysisName,photoName))
			...
			image = Image.open(os.path.join(self.script_dir, photoName))
			photo = ImageTk.PhotoImage(image, master=self.AnalysisPopup)
			
			# make label to display image in
			label = Label(self.AnalysisPopup, image=photo, bg="white")
			label.image = photo
			label.pack()
			

		elif self.AnalysisFrame.winfo_height() == 30:
			self.AnalysisFrame.config(height=200)
			self.AnalysisFrame.pack_configure(anchor='nw', fill='y')
		self.appendLog("Analysis Maximized")
			
	"""-----------------------------------------------END ANALYSIS WINDOW FRAMEWORK---------------------------------------------------"""
	
	"""--------------------------------------------------------ANALYSES---------------------------------------------------------------"""
	
	def nodeDegreesAnalysis(self):
		...
		# saves figure
		fig.savefig(os.path.join(self.script_dir,"histogramplot.png"), bbox_inches='tight') # Produce an image.
		
		self.AnalysisWindow("Node Degrees","histogramplot.png")
		
	def testAnalysis(self):
		self.AnalysisWindow("Test Analysis","horriblesmiley.png")
		
	def centralityAnalysis(self):
		kIn=[]
		kOut=[]
		k=0
		try:
			...
			self.appendLog("Error: No Nodes or No Central Node")
			return
	...
	def nodeDegrees(self):
		if len(self.G.nodes()) > 0 and \
		  ...
		  (not hasattr(self, 'nodeDegreePopup') or self.nodeDegreePopup.winfo_exists() == 0):
			...
			image = Image.open("histogramplot.png")
			photo = ImageTk.PhotoImage(image, master = self.nodeDegreeFrame)
	...
	def analysisMax(self):
		if self.frameOrWindow == 0 and self.nodeDegreeFrame.winfo_height() > 30:
			...
			fig.savefig("histogramplot.png", bbox_inches='tight')
			image = Image.open("histogramplot.png")
			photo = ImageTk.PhotoImage(image, master = self.nodeDegreePopup)
	...
	def logMax(self):
		if self.logFrameOrWindow == 0 and self.logFrame.winfo_height() > 30:
			...
			image = Image.open("exit.png")
			self.exitImage4 = ImageTk.PhotoImage(image, master = logPopUpToolbar)
			exitButton = Button(logPopUpToolbar, image=self.exitImage4, highlightbackground='light gray', command=self.logExit)
			image = Image.open("minimize.png")
			self.minImage4 = ImageTk.PhotoImage(image, master = logPopUpToolbar)
	...
	def showSubNetwork(self, node):
		...
		if (not hasattr(self, 'subNetworkFrame') or not self.subNetworkFrame.winfo_exists()) and \
		   (not hasattr(self, 'subNetworkPopUp') or self.subNetworkPopUp.winfo_exists() == 0) :
			...
			image = Image.open("exit.png") 
			self.exitImage5 = ImageTk.PhotoImage(image, master = self.subNetworktoolbar)
			...
			image = Image.open("minimize.png")
			self.minImage5 = ImageTk.PhotoImage(image, master = self.subNetworktoolbar)
			minButton = Button(self.subNetworktoolbar, image=self.minImage5, highlightbackground='light gray', command=self.subNetworkMin)
			image = Image.open("maximize.png")
			self.maxImage5 = ImageTk.PhotoImage(image, master = self.subNetworktoolbar)
	...
	def subNetworkMax(self):
		if self.SubNeworkFrameOrWindow == 0 and self.subNetworkFrame.winfo_height() > 30:
			...
			# creates images for toolbar buttons
			image = Image.open("exit.png")
			self.exitImage6 = ImageTk.PhotoImage(image, master = popUpToolbar)
			exitButton = Button(popUpToolbar, image=self.exitImage6, highlightbackground='light gray', command=self.subNetworkExit)
			image = Image.open("minimize.png")
			self.minImage6 = ImageTk.PhotoImage(image, master = popUpToolbar)
